tabItem(tabName = 'TargetFull',
	box(width=3, style='vertical-align:text:bottom;', 
		...
		sliderInput('targetFullRt', 'Select Rt', value=c(2, 11), min=0, max=20),
		...
		column(width=6, 
			numericInput('targetFullPrefilterL', 'prefilter level', value=10000),
			numericInput('targetFullNoise', 'noise', value=0)
		),		
		column(width=6, 
			numericInput('targetFullTolAbd', 'abundance tol (%)', value=20, min=0, max=100, step=1),
			numericInput('targetFullSnthresh', 'snthresh', value=4)
		),
